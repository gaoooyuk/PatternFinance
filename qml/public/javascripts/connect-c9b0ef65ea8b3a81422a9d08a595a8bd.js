...
function canvasDrawRotated(e, t, n, r) { e.translate(n[0], n[1]), e.rotate(t), r(), e.setTransform(1, 0, 0, 1, 0, 0) }
...
function scaleMeteors() { window.meteorAnimContainer || (window.meteorAnimContainer = document.querySelector(".page-header .animation"));
    var e = [-188, -138],
        t = [-148, -98],
        n = 600,
        r = 1050;
    ...
    i = clamp(i, 0, 1), meteorAnimContainer.style.marginTop = px(numInterpolate(e[0], e[1], i)), meteorAnimContainer.style.marginBottom = px(numInterpolate(t[0], t[1], i)) }(function() { "performance" in window == 0 && (window.performance = {}), Date.now = Date.now || function() {
    ...
    if ("now" in window.performance == 0) {
        ...
        performance.timing && performance.timing.navigationStart && (e = performance.timing.navigationStart), window.performance.now = function() {
            return Date.now() - e } } })(), "document" in self && ("classList" in document.createElement("_") ? function() { "use strict";
    ...
    u[n] = Error[n], l.item = function(e) {
        return this[e] || null }, l.contains = function(e) {
        return e += "", a(this, e) !== -1 }, l.add = function() {
        var e = arguments,
            t = 0,
            n = e.length,
            r, i = !1;
        do r = e[t] + "", a(this, r) === -1 && (this.push(r), i = !0); while (++t < n);
        i && this._updateClassName() }, l.remove = function() {
        var e = arguments,
            t = 0,
            n = e.length,
            r, i = !1,
            s;
        do { r = e[t] + "", s = a(this, r);
            while (s !== -1) this.splice(s, 1), i = !0, s = a(this, r) } while (++t < n);
        i && this._updateClassName() }, l.toggle = function(e, t) { e += "";
        var n = this.contains(e),
            r = n ? t !== !0 && "remove" : t !== !1 && "add";
        return r && this[r](e), t === !0 || t === !1 ? t : !n }, l.toString = function() {
        return this.join(" ") };
    if (i.defineProperty) {
        var h = { get: c, enumerable: !0, configurable: !0 };
        try { i.defineProperty(r, t, h) } catch (p) { p.number === -2146823252 && (h.enumerable = !1, i.defineProperty(r, t, h)) } } else i[n].__defineGetter__ && r.__defineGetter__(t, c) }(self));
var vec2 = {};
vec2.direction = function(e, t) {
    return [t[0] - e[0], t[1] - e[1]] }, vec2.length = function(e) {
    return Math.sqrt(e[0] * e[0] + e[1] * e[1]) }, vec2.distance = function(e, t) {
    var n = vec2.direction(e, t);
    return vec2.length(n) }, vec2.normalize = function(e, t) {
    return t === undefined && (t = vec2.length(e)), [e[0] / t, e[1] / t] }, vec2.scale = function(e, t) {
    return [e[0] * t, e[1] * t] }, vec2.add = function(e, t) {
    return [e[0] + t[0], e[1] + t[1]] }, vec2.subtract = function(e, t) {
    return [e[0] - t[0], e[1] - t[1]] }, vec2.angleVert = function(e) {
    return Math.atan2(e[0], e[1]) }, vec2.angleHorz = function(e) {
    return Math.atan2(e[1], e[0]) }, vec2.interpolate = function(e, t, n) {
    return [e[0] + (t[0] - e[0]) * n, e[1] + (t[1] - e[1]) * n] }, vec2.fromAngle = function(e, t) {
    return [Math.cos(e) * t, Math.sin(e) * t] };
var Meteors = function() {
        function N(t, n) {
            var r = { starA: { toSun: {}, tang: {}, toStarB: {} }, starB: { toSun: {}, tang: {} }, orbit: {}, progress: { current: 0 }, speed: 1, curvePoints: [] },
        ...
        function C(t, n) {
            ...
            r.className = "flare", r.style.left = px(t[0]), r.style.top = px(t[1]), r.style.borderColor = rgb(n), e.skyEl.appendChild(r), setTimeout(function() { e.skyEl.removeChild(r) }, 2e3) }
        ...
        function k(t) {
            for (var n = 0; n < e.meteorCurvePoints; n++) {
                ...
                else if (r < t.progress.startTimes.starB) var i = numReverseInterpolate(t.progress.startTimes.orbit, t.progress.startTimes.starB, r),
                    o = numInterpolate(t.orbit.enterAng, t.orbit.leaveAng, i),
                    u = vec2.fromAngle(o, t.orbit.r),
                    s = vec2.add(e.sun.c, u);
                else var i = numReverseInterpolate(t.progress.startTimes.starB, 1, r),
        ...
        function L(t) { e.ctx.globalAlpha = t.shineStrength * 6, canvasDrawRotated(e.ctx, t.curvePoints[t.progress.currentIndexClamped].fromSun.ang, e.sun.c, function() { e.ctx.drawImage(e.shineTemplCanvas, e.shineCanvasSize / -2, e.shineCanvasSize / -2) }) }

        function A(t) { e.ctx.strokeStyle = rgb([Math.round(numInterpolate(e.meteorColor[0][0], e.meteorColor[1][0], t.progress.current)), Math.round(numInterpolate(e.meteorColor[0][1], e.meteorColor[1][1], t.progress.current)), Math.round(numInterpolate(e.meteorColor[0][2], e.meteorColor[1][2], t.progress.current))]);
            ...
            for (var r = 0; r < e.meteorShades; r++) {
                ...
                e.ctx.beginPath(), e.ctx.moveTo(n.p[0], n.p[1]);
                for (var u = 1; u < o; u++) {
                ...
                l *= Math.max(t.shineStrength * 10, 0) + .5, e.ctx.globalAlpha = l, e.ctx.stroke() } }
        var e = this;
        e.skyEl = document.querySelector(".page-header .animation .sky"), e.skyParallaxEl = document.querySelector(".page-header .animation .sky-parallax"), e.container = document.querySelector(".page-header .animation .meteors"), e.width = 1200, e.height = 700, e.gridSize = [50, 50], e.offset = [11, 18], e.meteorInterval = [1e3, 3e3], e.meteorLimit = 4, e.meteorDuration = 8e3, e.meteorCurvePoints = 300, e.meteorShades = 8, e.meteorSegments = 16, e.meteorWidth = [2, 5], e.meteorAlpha = [.05, .12], e.meteorColor = [
            [185, 164, 255],
            [83, 236, 184]
        ], e.smallStars = 400, e.sun = { r: 80, c: [464, 369], strokeWidth: 2, attractRange: 70, pushRange: [97, 127, 177], shineRange: 250, shineColor: [204, 248, 255], shineSize: .6, shineSegments: 10, hazeColor: [134, 145, 255], hazeSize: 100 }, e.ringColor = "255,255,255", e.rings = [
            [97, .1],
            [127, .08],
            [177, .06],
            [254, .04],
            [361, .02]
        ], e.sky = [{ r: 7, strokeWidth: 4, strokeColor: rgb(e.meteorColor[0]), stars: [{ c: [3, 0], opacity: .1 }, { c: [0, 2], opacity: .2 }, { c: [2, 3], opacity: .3 }, { c: [4, 2], opacity: .3 }, { c: [1, 6], opacity: .3 }, { c: [2, 9], opacity: .3 }, { c: [8, 3], opacity: .4 }, { c: [4, 4], opacity: .4 }, { c: [5, 5], opacity: .4 }, { c: [4, 6], opacity: .4 }, { c: [5, 8], opacity: .4 }, { c: [7, 4], opacity: .5 }, { c: [7, 5], opacity: .5 }, { c: [6, 6], opacity: .5 }, { c: [6, 7], opacity: .5 }] }, { r: 10, strokeWidth: 2, strokeColor: rgb(e.meteorColor[1]), stars: [{ c: [15, 5], opacity: .5 }, { c: [15, 9], opacity: .5 }, { c: [14, 10], opacity: .5 }, { c: [13, 11], opacity: .5 }, { c: [16, 4], opacity: .4 }, { c: [17, 6], opacity: .4 }, { c: [17, 9], opacity: .4 }, { c: [16, 10], opacity: .4 }, { c: [17, 10], opacity: .4 }, { c: [16, 12], opacity: .4 }, { c: [18, 5], opacity: .3 }, { c: [18, 8], opacity: .3 }, { c: [19, 10], opacity: .3 }, { c: [22, 4], opacity: .2 }, { c: [23, 6], opacity: .2 }, { c: [21, 7], opacity: .2 }, { c: [21, 9], opacity: .2 }, { c: [23, 2], opacity: .2 }] }], /iPhone|iPod|iPad|Android/.test(navigator.userAgent) && (e.meteorCurvePoints /= 2, e.meteorSegments /= 2, e.meteorShades /= 2, e.meteorInterval[0] *= 2, e.meteorInterval[1] *= 2, e.meteorAlpha[0] *= 4, e.meteorAlpha[1] *= 1.5, e.meteorWidth[0] *= 1.5, e.meteorWidth[1] *= .8), e.cssPrefix = "animation" in document.body.style ? "" : "-webkit-", e.isPlaying = !1, e.lastTick = null, e.nextMeteor = -1, e.meteorLength = e.meteorSegments / e.meteorCurvePoints, e.meteors = [], e.avgMs = 0, e.frameCount = 0, e.canvas = document.createElement("canvas"), e.canvas.width = e.width, e.canvas.height = e.height, e.ctx = e.canvas.getContext("2d"), e.ctx.lineCap = "round", e.container.appendChild(e.canvas);
        var t = window.devicePixelRatio,
            n = e.sun.r * 2 + 20,
            r = document.createElement("canvas");
        r.width = r.height = n * t, r.style.width = r.style.height = px(n), r.style.left = px(e.sun.c[0] - n / 2), r.style.top = px(e.sun.c[1] - n / 2);
        var i = r.getContext("2d");
        i.scale(t, t), e.skyEl.appendChild(r);
        var s = i.createLinearGradient(0, 0, n, n);
        s.addColorStop(.1, rgba(e.meteorColor[0], .65)), s.addColorStop(.9, rgba(e.meteorColor[1], .65)), canvasCircle(i, [n / 2, n / 2], e.sun.r), i.lineWidth = e.sun.strokeWidth, i.strokeStyle = s, i.stroke();
        for (var o = 0; o < e.sky.length; o++) {
            var u = e.sky[o];
            for (var a = 0; a < u.stars.length; a++) {
                var f = u.stars[a];
                f.c[0] = Math.round(f.c[0] * e.gridSize[0] + e.offset[0]), f.c[1] = Math.round(f.c[1] * e.gridSize[1] + e.offset[1]);
                var l = document.createElement("div");
                l.className = "star", l.style.width = l.style.height = px(u.r * 2), l.style.left = px(f.c[0] - u.r), l.style.top = px(f.c[1] - u.r), l.style.border = u.strokeWidth + "px solid " + u.strokeColor, l.style.opacity = f.opacity, l.style[e.cssPrefix + "animation-name"] = arrayRandom(["star1", "star2"]), l.style[e.cssPrefix + "animation-duration"] = numInterpolate(2e3, 1e4, Math.random()) + "ms", l.style[e.cssPrefix + "animation-delay"] = numInterpolate(0, 1e3, Math.random()) + "ms", e.skyEl.appendChild(l) } }
        for (var o = 0; o < e.rings.length; o++) {
            var c = e.rings[o],
                h = document.createElement("div");
            h.className = "ring", h.style.width = h.style.height = px(c[0] * 2), h.style.left = px(e.sun.c[0] - c[0]), h.style.top = px(e.sun.c[1] - c[0]), h.style.opacity = c[1], h.style.border = "1px solid rgb(" + e.ringColor + ")", e.skyEl.appendChild(h) }
        var p = e.sun.r,
            d = e.sun.r + 20,
            v = 500;
        while (e.smallStars--) {
            var m = 10 + Math.random() * (e.width - 20),
                ...
                w = Math.pow(y * b, 2);
            if (m > e.sun.c[0] - e.sun.r && m < e.sun.c[0] + e.sun.r && g > e.sun.c[1] - e.sun.r && g < e.sun.c[1] + e.sun.r || Math.random() > w) continue;
            ...
            l.className = "dot", l.style.width = l.style.height = px(Math.floor(Math.random() * 3 + 4)), l.style.left = px(m), l.style.top = px(g), l.style.backgroundColor = E, l.style.opacity = Math.random() * .25 + .25, l.style[e.cssPrefix + "animation-name"] = arrayRandom(["dot1", "dot2", "dot3", "dot4"]), l.style[e.cssPrefix + "animation-duration"] = numInterpolate(1e4, 4e4, Math.random()) + "ms", l.style[e.cssPrefix + "animation-delay"] = numInterpolate(0, 1e3, Math.random()) + "ms", e.skyParallaxEl.appendChild(l) }
        ...
        s.addColorStop(0, rgba(e.sun.hazeColor, 1)), s.addColorStop(.25, rgba(e.sun.hazeColor, .65)), s.addColorStop(.5, rgba(e.sun.hazeColor, .37)), s.addColorStop(.75, rgba(e.sun.hazeColor, .16)), s.addColorStop(1, rgba(e.sun.hazeColor, 0)), e.shineTemplCtx.fillStyle = s, e.shineTemplCtx.globalAlpha = .25, e.shineTemplCtx.fillRect(0, 0, e.shineCanvasSize, e.shineCanvasSize), e.shineTemplCtx.globalAlpha = 1, e.shineTemplCtx.lineWidth = e.sun.strokeWidth * 1.5, e.shineTemplCtx.strokeStyle = rgba(e.sun.shineColor, .15);
        for (var a = 0; a < e.sun.shineSegments; a++) {
            var S = a / e.sun.shineSegments,
                x = e.sun.shineSize * (1 - S);
            e.shineTemplCtx.beginPath(), e.shineTemplCtx.arc(e.shineCanvasSize / 2, e.shineCanvasSize / 2, e.sun.r, -x, x), e.shineTemplCtx.stroke() }
        ...
        e.tick = function(t) { t || (t = e.lastTick), e.delta = Math.min(t - e.lastTick, 50);
            ...
            e.lastTick = t, e.isPlaying && requestAnimationFrame(e.tick) }, e.start = function() {
            if (e.isPlaying) return;
            e.lastTick = performance.now(), e.isPlaying = !0, e.tick() }, e.stop = function() {
            if (!e.isPlaying) return;
            e.isPlaying = !1 } },
    Feature = function() {
        var e = this;
        e.interval = 5500, e.current = 0, e.change = function() { requestAnimationFrame(function() {
...
$(function(e) {
    var t = document.querySelector(".page-header").getBoundingClientRect().height,
        ...
        r = new Meteors;
    window.headerMeteors = r, scaleMeteors(), window.addEventListener("resize", function(e) { scaleMeteors() }), window.addEventListener("blur", function(e) { r.stop() }), window.addEventListener("focus", function(e) { r.start() });
    var i = new Feature;
    ...
    o.forEach(function(e, t) { o[t] = document.querySelector(e), o[t].classList.add("uncover-ready") });
    ...
    window.addEventListener("scroll", function(e) { requestAnimationFrame(function() { window.scrollY > t ? r.stop() : r.start(), o.forEach(function(e) {
            ...
            if (u) {
                ...
                if (e.top + e.height < 0 || e.top > window.innerHeight) u.className = "currencies icon-" + Math.ceil(Math.random() * 10) } }) }) });
