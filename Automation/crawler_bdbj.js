	...
	async.parallel([
	    function(callback) {
	    	if (queueColumn.length > 0) {
	    		...
	    		crawlBdbjColumn(queueColumn.shift(), callback)
	    	} else {
	    ...
	    function(callback) {
	    	if (queueArticle.length > 0) {
	    		...
	    		crawlBdbjArticle(queueArticle.shift(), callback)
	    	} else {
	    		// if we run out of article urls
				// wait for crawlBdbjColumn to fulfill 
	    	}
	    }
	],
...
var crawlBdbjColumn = function(url, cb) {
    ...
    phantomJS.createPage().then(page => {
        ...
        page.open(url).then(status => {
        	...
        	var ds = url.split(".")
        	if (ds) {
        		var userId = ds[0].replace("http://", "")
        		writerId = idHash[userId]
        	}

        	var pv = document.querySelector(".author-data .page-view b")
        	if (pv) {
        		var totalViewedTimes = parseInt(String(pv.innerHTML).replace(/,/g, ""))
        		console.log("totalViewedTimes: ", totalViewedTimes)
        	}

        	var an = document.querySelector(".author-data .article-num b")
        	if (an) {
        		var totalNumArticles = parseInt(String(an.innerHTML).replace(/,/g, ""))
			...
			var articles = []
			var feeds = document.querySelector("div.authorFeeds")
			if (feeds) {
				var nodelist = feeds.querySelectorAll(".feeds-item")
	        	var posts = Array.prototype.slice.call(nodelist)
	        	
				for (p in posts) {
					var article = {}
					var e = posts[p]
					var link = e.querySelector('h3 > a')
					...
					// TODO:
					var t = e.querySelector(".point-box .time")
					if (t) {
						article.timestamp = t.innerHTML
					}
					
					var vtext = e.querySelector('.feeds-item-info .count').innerHTML
					vtext = vtext.replace("阅读 (", "").replace(")", "")
...
var crawlBdbjArticle = function(url, cb) {
	console.log("crawl: ", url)
	var article = {}
    var ls = url.split("/")
    article.originId = ls[ls.length - 1]
	...
	phantomJS.createPage().then(page => {
        ...
        page.open(url).then(status => {
        	...
        	var document = jsdom.jsdom(content);
        	var status = document.querySelector(".artical-status")
        	if (status) {
        		article.viewedTimes = parseInt(status.querySelector(".read .readnum").innerHTML)
        		var updown = status.querySelector(".up_down")
        		if (updown) {
        			var up = parseInt(updown.querySelector("#up_article .num").innerHTML)
        			var down = parseInt(updown.querySelector("#down_article .num").innerHTML)
        			var liked = up - down
        			article.likedTimes = liked > 0 ? liked : 0
        		}
        	}
        	var cmtnum = document.querySelector(".cmtnum")
        	if (cmtnum) {
        		article.commentedTimes = parseInt(cmtnum.innerHTML)
        	}
...
function getUrls(cb) {
	async.parallel([
				...
				assert.equal(err, null);
			  	...
			  	if (doc != null) {
			  		for (var i = 0; i < doc.medias.length; i++) {
			  			...
			  			if ("bdbj" === mt.channel) {
			  				idHash[mt.id] = doc.id
			  				queueColumnRoot.push("http://" + mt.id + ".baijia.baidu.com/")
			  				break
			  			}
			  		}
			  	} else {
	    ...
	    function(callback) {
	    	// queueArticleRoot.push("http://qijunjie.baijia.baidu.com/article/815446")
			// queueArticleRoot.push("http://mediachanged.baijia.baidu.com/article/810269")
			// queueArticleRoot.push("http://yemacaijing.baijia.baidu.com/article/814136")
	    	// queueArticleRoot.push("http://zhuojimin.baijia.baidu.com/article/814209")
	    }
	],
	// async.parallel callback
	function(err, results) {
